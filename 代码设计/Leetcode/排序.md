           ...
           for (int outer =1; outer<=upper; outer++)
            {
                ...
                for (int iinner = 0; iinner <= upper-outer; iinner++)
                {
                    if (arr[iinner] > arr[iinner + 1])
                    {
                        temp = arr[iinner];
                        arr[iinner] = arr[iinner + 1];
                        arr[iinner + 1] = temp;
                    }
                }
            }
...
特点：初始数组不影响排序；数据移动最少（n次）；比较次数：n^2/2
...
public void SelectionSort(int[] a)
        {
            ...
            for (int outer = 0; outer <= upper; outer++)
            {
                minIndex = outer;
                //找到最小值索引
                for (int iinner = outer + 1; iinner <= upper; iinner++)
                {
                    if (arr[iinner] < arr[minIndex])
                        minIndex = iinner;
                }
                //交换
                temp = arr[outer];
...
初始数组影响排序；数据移动：n\^2/2；比较次数：n^2/4；只能相邻比较，对于大规模乱序效率较低
 ...
 public void InsertSort(int[] a)
        {
            ...
            for (int outer = 1; outer <= upper; outer++)
            {
                temp = arr[outer];
                iinner = outer;
                while (iinner > 0 && arr[iinner - 1] >= temp)  //数据后移
                {
                    arr[iinner] = arr[iinner - 1];
                    iinner -= 1;
                }
...
public void ShellSort()
        {
            ...
            while (h > 0)
            {
                for (int outer = h; outer <= upper; outer++)
                {
                    temp = arr[outer];
                    iinner = outer;
                    while ((iinner > h - 1) && arr[iinner - h] >= temp)
                    {
                        arr[iinner] = arr[iinner - h];
                        iinner -= h;
                    }
                    arr[iinner] = temp;
                }
